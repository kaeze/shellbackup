...
server_farm(){
ssh 10.113.128.4 -l 'htctaipei\kyle_lin'
}
skip_oobe(){
    adb shell settings put global device_provisioned 1
    adb shell settings put secure user_setup_complete 1
    adb shell settings put system setup_wizard_has_run 1
    adb shell pm disable com.google.android.setupwizard
}
enable_htc_logs(){
    adb shell 'echo 1C187F1818181818181818181818181818181818181818181818181818181818 > /etc/hldm.bin'
}

setfps(){
      adb shell "service call SurfaceFlinger 3007 i32 $1"
}
export errorpattern="FATAL EXCEPTION|WATCHDOG TIMEOUT|Exit zygote|OutOfMemoryError|App crashed| WATCHDOG KILLING|ANR in|CORE_DUMP_DETECT = 1|Camera Control: Test Activity not resume| failed to come online|ServiceManager: Waiting for service|VoldConnector"

systrace_wake_up_issue(){
    sed -i s#success=\(.d\+\)\ #success=\(\\\d\+\)\ \)\?#g $1
}

update_mytask()
{
    firefox --new-window  http://pqm.ssd.htc.com.tw:85/LogDailyTask.aspx && sleep 2 && xmacroplay -d 2000 "$DISPLAY" < ~/bin/auto_mytask2.src
}
